...
public abstract class DropDownFilterDialog extends Dialog {
    ...
    public void constructButton(ArrayList<DropDownItem> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            TextView btn = (TextView) view.findViewById(R.id.top_menu_button);
            btn.setText(item.getName());
            btn.setTag(item.getIndex());
            if (mCurrentIndex == i) {
                btn.setTextColor(mContext.getResources().getColor(R.color.drop_down_filter_menu_color_selected));
            }
